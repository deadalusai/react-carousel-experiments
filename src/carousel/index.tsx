import * as React from "react";

import { Animator, AnimationHandle } from "./animator";

const ANIMATION_TIME_MS = 300; // ms

export interface ICarouselProps {
    children: React.ReactNode[];
}

interface ICarouselState {
    scrollIndex: number;
}

export class Carousel extends React.Component<ICarouselProps, ICarouselState> {
    state = { scrollIndex: 0 };

    animator = new Animator();
    animation = null as AnimationHandle | null;
    container = null as HTMLDivElement | null;

    public render() {
        
        const itemWidth = () => {
            const scrollWidth = this.container?.scrollWidth ?? 0;
            return scrollWidth / this.props.children.length; // NOTE: Assuming all items are of equal width
        };
        const animateScroll = (targetScrollIndex: number) => {
            if (!this.container) {
                return;
            }
            if (this.animation) {
                this.animation.cancel();
            }
            const scrollLeft = targetScrollIndex * itemWidth();
            this.animation = this.animator.scrollToLeft(this.container, scrollLeft, ANIMATION_TIME_MS);
            this.animation.complete.then((completed) => {
                if (completed) {
                    this.animation = null;
                }
            });
        };
        const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
            if (this.animation) {
                return;
            }
            const scrollIndex = Math.floor(e.currentTarget.scrollLeft / itemWidth());
            this.setState({ scrollIndex });
        };
        const goLeft = (e: React.MouseEvent) => {
            e.preventDefault();
            e.stopPropagation();
            let scrollIndex = this.state.scrollIndex - 1;
            if (scrollIndex < 0) {
                scrollIndex = 0;
            }
            this.setState({ scrollIndex });
            animateScroll(scrollIndex);
        };
        const goRight = (e: React.MouseEvent) => {
            e.preventDefault();
            e.stopPropagation();
            let scrollIndex = this.state.scrollIndex + 1;
            if (scrollIndex >= this.props.children.length) {
                scrollIndex = this.props.children.length - 1;
            }
            this.setState({ scrollIndex });
            animateScroll(scrollIndex);
        };
        return <div className="bil-carousel">
            <div className="bil-carousel--container" onScroll={handleScroll} ref={(e) => this.container = e}>
                {this.props.children.map((c, i) =>
                    <div key={i} className="bil-carousel--item">
                        {c}
                    </div>
                )}
            </div>
            <div className="bil-carousel--controls">
                <button className="bil-carousel--button" onClick={goLeft}>
                    Prev
                </button>
                <button className="bil-carousel--button" onClick={goRight}>
                    Next
                </button>
            </div>
        </div>;
    }
}
